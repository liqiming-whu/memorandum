...
如果不指定安装目录，Python 3.7 默认安装在了/usr/local/bin文件夹中，可运行文件/usr/local/bin，库文件/usr/local/lib。因为 /usr/local/bin 在 Shell 路径中，所以可以直接在 Shell 中输入 python3 运行 Python 3.7 解释器
